    ...
    if (m_keyRoleFilter.isEmpty()) {
        ...
        if (!m_dataSource->data()->isEmpty()) {
            ...
            for (const QString &key : lst) {
                ...
                if (value.isValid() && value.canConvert<Lingmo5Support::DataEngine::Data>()) {
                    Lingmo5Support::DataEngine::Data data = value.value<Lingmo5Support::DataEngine::Data>();
...
void DataModel::setDataSource(QObject *object)
{
    ...
    for (const QString &key : keys) {
        dataUpdated(key, m_dataSource->data()->value(key).value<Lingmo5Support::DataEngine::Data>());
    }
