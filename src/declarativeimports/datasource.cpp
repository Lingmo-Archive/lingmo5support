...
DataSource::DataSource(QObject *parent)
    ...
    , m_ready(false)
    , m_interval(0)
    , m_intervalAlignment(Lingmo5Support::Types::NoAlignment)
{
...
void DataSource::setEngine(const QString &e)
{
    ...
    m_dataEngineConsumer.reset(new Lingmo5Support::DataEngineConsumer());
    Lingmo5Support::DataEngine *engine = dataEngine(m_engine);
    if (!engine) {
...
void DataSource::setIntervalAlignment(Lingmo5Support::Types::IntervalAlignment intervalAlignment)
{
...
void DataSource::dataUpdated(const QString &sourceName, const Lingmo5Support::DataEngine::Data &data)
{
...
void DataSource::modelChanged(const QString &sourceName, QAbstractItemModel *model)
{
    ...
    connect(model, &QObject::destroyed, m_models, [this, sourceName]() {
        m_models->clear(sourceName);
    });
}
...
void DataSource::removeSource(const QString &source)
{
    ...
    if (m_dataEngine) {
        QHash<QString, Lingmo5Support::Service *>::iterator it = m_services.find(source);
...
QObject *DataSource::serviceForSource(const QString &source)
{
    if (!m_services.contains(source)) {
        Lingmo5Support::Service *service = m_dataEngine->serviceForSource(source);
        if (!service) {
            return nullptr;
        }
