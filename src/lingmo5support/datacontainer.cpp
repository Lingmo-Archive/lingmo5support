...
void DataContainer::connectVisualization(QObject *visualization, uint pollingInterval, Lingmo5Support::Types::IntervalAlignment alignment)
{
    // qCDebug(LOG_LINGMO) << "connecting visualization" <<this<< visualization << "at interval of"
    ...
    if (connected) {
        ...
        if (relay) {
            ...
            // qCDebug(LOG_LINGMO) << "     already connected, but to a relay";
            if (relay->m_interval == pollingInterval && relay->m_align == alignment) {
                // qCDebug(LOG_LINGMO) << "    already connected to a relay of the same interval of"
                //          << pollingInterval << ", nothing to do";
                return;
            }

            if (relay->receiverCount() == 1) {
                // qCDebug(LOG_LINGMO) << "    removing relay, as it is now unused";
                d->relays.remove(relay->m_interval);
                delete relay;
            } else {
                if (visualization->metaObject()->indexOfSlot("dataUpdated(QString,Lingmo5Support::DataEngine::Data)") >= 0) {
                    disconnect(relay,
                               SIGNAL(dataUpdated(QString, Lingmo5Support::DataEngine::Data)),
                               visualization,
                               SLOT(dataUpdated(QString, Lingmo5Support::DataEngine::Data)));
                }
            ...
            // nothing to do!
            // qCDebug(LOG_LINGMO) << "     already connected, nothing to do";
            return;
        } else {
            if (visualization->metaObject()->indexOfSlot("dataUpdated(QString,Lingmo5Support::DataEngine::Data)") >= 0) {
                disconnect(this,
    ...
    if (pollingInterval < 1) {
        // qCDebug(LOG_LINGMO) << "    connecting directly";
        ...
        // qCDebug(LOG_LINGMO) << "    connecting to a relay";
...
{
    // qCDebug(LOG_LINGMO) << objectName() << d->dirty;
    if (d->dirty) {
...
{
    return !d->relays.isEmpty() || receivers(SIGNAL(dataUpdated(QString, Lingmo5Support::DataEngine::Data))) > 0;
}
...
void DataContainer::timerEvent(QTimerEvent *event)
{
    if (event->timerId() == d->checkUsageTimer.timerId()) {
        if (!isUsed()) {
            ...
            // qCDebug(LOG_LINGMO) << objectName() << "is unused";
