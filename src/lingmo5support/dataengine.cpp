...
#include "config-lingmo5support.h"
...
void DataEngine::connectSource(const QString &source,
                               ...
                               Lingmo5Support::Types::IntervalAlignment intervalAlignment) const
{
    // qCDebug(LOG_LINGMO) << "connectSource" << source;
    bool newSource;
    ...
    if (s) {
        ...
        // qCDebug(LOG_LINGMO) << " ==> source connected";
    }
}

void DataEngine::connectAllSources(QObject *visualization, uint pollingInterval, Lingmo5Support::Types::IntervalAlignment intervalAlignment) const
{
...
void DataEngine::setModel(const QString &source, QAbstractItemModel *model)
{
    ...
    Lingmo5Support::DataContainer *s = containerForSource(source);
...
#ifndef NDEBUG
        // qCDebug(LOG_LINGMO) << "source named \"" << source->objectName() << "\" already exists.";
#endif
        return;
    }
...
void DataEngine::removeAllSources()
{
    ...
    QMutableHashIterator<QString, Lingmo5Support::DataContainer *> it(d->sources);
    while (it.hasNext()) {
        it.next();
        Lingmo5Support::DataContainer *s = it.value();
        it.remove();
        s->disconnect(this);
        s->deleteLater();
    }
...
void DataEngine::timerEvent(QTimerEvent *event)
{
    ...
    if (event->timerId() == d->updateTimerId) {
        ...
        if (d->minPollingInterval < 0) {
            // qCDebug(LOG_LINGMO) << "uh oh.. no polling allowed!";
            return;
        }

        // minPollingInterval
        if (d->updateTimer.elapsed() < d->minPollingInterval) {
            // qCDebug(LOG_LINGMO) << "hey now.. slow down!";
            return;
        }
        ...
        QHashIterator<QString, Lingmo5Support::DataContainer *> it(d->sources);
        while (it.hasNext()) {
            it.next();
...
void DataEngine::updateAllSources()
{
    ...
    while (it.hasNext()) {
        it.next();
        // qCDebug(LOG_LINGMO) << "updating" << it.key();
        if (it.value()->isUsed()) {
...
void DataEnginePrivate::internalUpdateSource(DataContainer *source)
{
    if (minPollingInterval > 0 && source->timeSinceLastUpdate() < (uint)minPollingInterval) {
        ...
        // qCDebug(LOG_LINGMO) << "internal update source is delaying" << source->timeSinceLastUpdate() << minPollingInterval;
    ...
    if (q->updateSourceEvent(source->objectName())) {
        // qCDebug(LOG_LINGMO) << "queuing an update";
...
DataContainer *DataEnginePrivate::source(const QString &sourceName, bool createWhenMissing)
{
    ...
    // qCDebug(LOG_LINGMO) << "DataEngine " << q->objectName() << ": could not find DataContainer " << sourceName << ", creating";
...
void DataEnginePrivate::connectSource(DataContainer *s,
                                      QObject *visualization,
                                      uint pollingInterval,
                                      Lingmo5Support::Types::IntervalAlignment align,
                                      bool immediateCall)
{
    // qCDebug(LOG_LINGMO) << "connect source called" << s->objectName() << "with interval" << pollingInterval;
    ...
    if (immediateCall) {
        ...
        // reconnecting
        // qCDebug(LOG_LINGMO) << "immediate call requested, we have:" << s->visualizationIsConnected(visualization);
    ...
    if (immediateCall) {
        QMetaObject::invokeMethod(visualization, "dataUpdated", Q_ARG(QString, s->objectName()), Q_ARG(Lingmo5Support::DataEngine::Data, s->data()));
        if (s->d->model) {
