...
{
    const static QString s_servicesPluginDir = QStringLiteral("lingmo5support/services");
    ...
    if (plugin.isValid()) {
        service = KPluginFactory::instantiatePlugin<Lingmo5Support::Service>(plugin, parent).plugin;
    }
...
{
    const static QString s_dataEnginePluginDir = QStringLiteral("lingmo5support/dataengine");
