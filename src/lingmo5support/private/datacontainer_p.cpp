...
SignalRelay::SignalRelay(DataContainer *parent, DataContainerPrivate *data, uint ival, Lingmo5Support::Types::IntervalAlignment align, bool immediateUpdate)
    : QObject(parent)
    , dc(parent)
    , d(data)
    , m_interval(ival)
    ...
    , m_queued(true)
{
    // qCDebug(LOG_LINGMO) << "signal relay with time of" << m_timerId << "being set up";
    m_timerId = startTimer(immediateUpdate ? 0 : m_interval);
    if (m_align != Lingmo5Support::Types::NoAlignment) {
        checkAlignment();
    }
}
...
{
    return receivers(SIGNAL(dataUpdated(QString, Lingmo5Support::DataEngine::Data)));
}

bool SignalRelay::isUnused() const
{
    return receivers(SIGNAL(dataUpdated(QString, Lingmo5Support::DataEngine::Data))) < 1;
}
...
void SignalRelay::checkAlignment()
{
    ...
    if (m_align == Lingmo5Support::Types::AlignToMinute) {
        int seconds = t.second();
        newTime = ((60 - seconds) * 1000) + 500;
    } else if (m_align == Lingmo5Support::Types::AlignToHour) {
...
void SignalRelay::timerEvent(QTimerEvent *event)
{
    ...
    if (d->hasUpdates()) {
        // qCDebug(LOG_LINGMO) << "emitting data updated directly" << d->data;
        Q_EMIT dataUpdated(dc->objectName(), d->data);
        m_queued = false;
    } else {
