...
public:
    ...
    ~DataEngineManagerPrivate()
    {
        for (Lingmo5Support::DataEngine *engine : std::as_const(engines)) {
            delete engine;
        }
        engines.clear();
        delete nullEng;
...
Lingmo5Support::DataEngine *DataEngineManager::engine(const QString &name) const
{
    if (name.isEmpty()) {
        return d->nullEngine();
    }

    Lingmo5Support::DataEngine::Dict::const_iterator it = d->engines.constFind(name);
...
Lingmo5Support::DataEngine *DataEngineManager::loadEngine(const QString &name)
{
    if (name.isEmpty()) {
        qCDebug(LOG_LINGMO5SUPPORT) << "Asked an engine with empty name";
        return d->nullEngine();
    }
    Lingmo5Support::DataEngine::Dict::const_iterator it = d->engines.constFind(name);
    ...
    const KPluginMetaData data(QLatin1String("lingmo5support/dataengine/lingmo_engine_") + name);
    DataEngine *engine = KPluginFactory::instantiatePlugin<Lingmo5Support::DataEngine>(data).plugin;
    if (!engine) {
        qCDebug(LOG_LINGMO5SUPPORT) << "Can't find a dataengine named" << name;
        return d->nullEngine();
    }
...
{
    Lingmo5Support::DataEngine::Dict::iterator it = d->engines.find(name);

    if (it != d->engines.end()) {
        Lingmo5Support::DataEngine *engine = *it;
        engine->d->deref();
...
{
#ifndef NDEBUG
    QString path = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation) + QStringLiteral("/lingmo_dataenginemanager_log");
    QFile f(path);
    if (!f.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
        // qCDebug(LOG_LINGMO) << "failed to open" << path;
        return;
    }
    ...
    while (it.hasNext()) {
        ...
        for (DataContainer *dc : lst) {
            ...
            const int directs = dc->receivers(SIGNAL(dataUpdated(QString, Lingmo5Support::DataEngine::Data)));
            if (directs > 0) {
