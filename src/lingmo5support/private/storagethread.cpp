...
Lingmo5Support::StorageThread *StorageThread::self()
{
...
{
    if (!m_db.open()) {
        m_db = QSqlDatabase::addDatabase(QStringLiteral("QSQLITE"), QStringLiteral("lingmo-storage-%1").arg((quintptr)this));
        const QString storageDir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
        QDir().mkpath(storageDir);
        m_db.setDatabaseName(storageDir + QLatin1Char('/') + QStringLiteral("lingmo-storage2.db"));
    }

    if (!m_db.open()) {
        qCWarning(LOG_LINGMO5SUPPORT) << "Unable to open the lingmo storage cache database: " << m_db.lastError();
        ...
        if (!query.exec()) {
            qCWarning(LOG_LINGMO5SUPPORT) << "Unable to create table for" << caller->clientName();
            m_db.close();
        }
    }
    m_db.transaction();
...
void StorageThread::save(QPointer<StorageJob> wcaller, const QVariantMap &params)
{
    ...
    while (it.hasNext()) {
        it.next();
        // qCDebug(LOG_LINGMO) << "going to insert" << valueGroup << it.key();
        ...
        if (!query.exec()) {
            // qCDebug(LOG_LINGMO) << "query failed:" << query.lastQuery() << query.lastError().text();
            m_db.commit();
            Q_EMIT newResult(caller, false);
            return;
        }
