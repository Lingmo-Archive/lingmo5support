...
#ifndef NDEBUG
        // qCDebug(LOG_LINGMO5SUPPORT) << "No valid operations scheme has been registered";
#endif
        return QStringList();
    }
...
QVariantMap Service::operationDescription(const QString &operationName)
{
    ...
    // qCDebug(LOG_LINGMO5SUPPORT) << "operation" << operationName
...
#ifndef NDEBUG
            // qCDebug(LOG_LINGMO5SUPPORT) << "Operation" << op << "is disabled";
#endif
        } else {
...
#ifndef NDEBUG
        // qCDebug(LOG_LINGMO5SUPPORT) << op << "is not a valid group; valid groups are:" << d->operationsMap.keys();
#endif
    }
...
void Service::registerOperationsScheme()
{
    ...
    if (d->name.isEmpty()) {
#ifndef NDEBUG
        // qCDebug(LOG_LINGMO5SUPPORT) << "No name found";
#endif
        return;
    }
    ...
    const QString path =
        QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                               QStringLiteral(LINGMO5SUPPORT_RELATIVE_DATA_INSTALL_DIR "/services/") + d->name + QStringLiteral(".operations"));

    if (path.isEmpty()) {
#ifndef NDEBUG
        // qCDebug(LOG_LINGMO5SUPPORT) << "Cannot find operations description:" << d->name << ".operations";
#endif
        return;
    }
